...
void rms_current(uint8_t motor_id, uint16_t mA)
{
    ...
    if (cs < 16)
    {
        tmc2209_vsense(motor_id, true); // Enable vsense for lower current settings
        cs = 32.0 * 1.41421 * mA / 1000.0 * (Rsense + 0.02) / VFS_HighSensitivy_mA - 1;
    }
    else
    {
        tmc2209_vsense(motor_id, false); // Disable vsense for higher current settings
    }
    if (cs > 31)
...
{
    tmc2209_en_SpreadCycle(motor_id, !enable); // SpreadCycle is the default mode, so we invert the logic here
}
